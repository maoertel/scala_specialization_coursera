...
object Huffman {
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def sub(codeTree: CodeTree, bits: List[Bit]): List[Char] =
      codeTree match {
        case Leaf(char, _) if bits.isEmpty => List(char)
        case Leaf(char, _) => char :: sub(tree, bits)
        case Fork(left, _, _, _) if bits.head == 0 => sub(left, bits.tail)
        case Fork(_, right, _, _) => sub(right, bits.tail)
      }
